<template lang="pug">
  div
    ...
    div.panel.options-panel
      table
        tbody
          tr#eyedropper-options
            th {{ 'BOXING' }}
              td
                el-input#boxing-notification.cz-autostart(type="checkbox" value="1")
                  label(for="boxing-notification-label" class="cz-autostart") {{ 'Be notified of boxing events.' }}
                  br.cz-autostart
          tr#color-format-options
             th {{'MMA'}}
               td
                 el-input#mma-notification(type="checkbox" value="1")
                 label(id="mma-notification-label" for="mma-notification") {{'Be notified of MMA events.'}}
                 br
...
.panel.heading img { float: left; margin-left: -5px; }

.options-panel { line-height: 2em; }
.options-panel th,
.options-panel td { vertical-align: top; white-space: nowrap; }
.options-panel th { text-align: left; width: 1%; padding: 8px 15px 10px 10px; }
.options-panel td { width: 99%; padding: 10px; }
.options-panel label.indent { margin-left: 24px; }

#footer { margin: 10px 15px; }
#save-button { font-weight: bold; }
#status { margin-left: 5px; color: #02893D; font-weight: bold; }

#about-panel { line-height: 1.5em;}
#about-panel h2 { font-size: 15px; padding:0; margin:0; margin: 18px 0 3px 0; }
#about-panel h2.first { margin-top: 0; } 
</style>